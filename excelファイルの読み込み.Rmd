...
#### スペースや記号などを自動的に変換してくれる関数できれいに {#re-hidoi-ss}
...
「パイプ」と読みます^[2022年4月以前は`%>%`が使われることが多かったのですが，最近では`|>`が用いられるケースが増えています。]。処理を重ねてコードに書いていきたい際に重宝し、現代の`tidyverse`を使ったRのコードに欠かせないものです。たとえば、データフレームdfのspecies列を選択する、という処理の
...
なお、ショートカット`ctrl + shit + M`（Macだと`	Cmd + Shift + M`）で出せます^[現時点ではRStudioのデフォルトのショートカットで出るのは`%>%`だが，`|>`に切り替えたい場合は，RStudioの`Tools > Global Options > Code > Editing > use native pipe operator`にチェックを入れます]。


#### 全角←→半角を自動で {#re-hidoi-zh}
...
# ここ1回のみでしかstri_trans_nfkc()を使わないのであれば，パッケージ名を指定して読みだす::を使用して，library(stringi)は書かず，stringi::stri_trans_nfkc()と書いてもよいです。

```

\newpage

#### 上記の合わせ技 {#re-hidoi-zhcomb}
...
## シートを指定して読み込む {#re-sheet}
### シート名の確認 {#re-sheetname}
...
## 複数のExcelファイルを読み込む {#re-multiple-excel}
...
### 読み込むファイル名の一覧のオブジェクト作成 {#re-filename}
...
#### リストの要素名にファイル名を付与 {#re-add-filename}
...
### 読み込むファイルが複数フォルダにある場合 {#re-multiple-dir}
