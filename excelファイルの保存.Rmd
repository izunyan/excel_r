...
なお、ここで`knitr::kable()`という書き方が出てきますが、これは`knitr`パッケージの`kable()`関数という意味です。関数を1回しか使わないのに`library(knitr)`で呼び出すのは無駄が多かったりするので、時々こういった記述が出てきますので、覚えておいてください。この書き方は、パッケージ間で同じ名前の関数があったときにぶつかってしまう問題を回避する際にも使われたりします。
